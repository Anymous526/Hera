...
//		days: ['日', '一', '二', '三', '四', '五', '六'],
		...
		tweak: {x: 10, y: 10} // tweak calendar positioning
	},
	...
	initialize: function(obj, options) {
		...
		for (var i in obj) {
			...
			// create cal button
			cal.button.addClass(this.classes.calendar).addEvent('click', function(cal) { this.toggle(cal); cal.visible = true;}.pass(cal, this)).injectAfter(cal.el);
			cal.button.addClass(this.classes.calendar).addEvent('click', function(cal) { this.toggle(cal); cal.visible = true;}.pass(cal, this)).injectAfter(cal.el);
	...
	bounds: function(cal) {
		...
		var date = new Date().getDate() + this.options.direction.toInt()-1;
	...
	display: function(cal) {
		...
		var bnt_div = new Element('div',{'style':'top:-10px'});
		bnt_div.inject(div);
		var bnt_ul = new Element('ul');
		bnt_ul.inject(bnt_div);
		var bnt_li = new Element('li');
		bnt_li.inject(bnt_ul);
		
		var bnt_today = new Element('button',{'id':'bnt_today','type':'button','text':'今天'});
		bnt_today.injectInside(bnt_li);
		var bnt_clear = new Element('button',{'id':'bnt_clear','type':'button','text':'清空'});
		bnt_clear.injectInside(bnt_li);

//		$('bnt_today').setStyle('cursor':'pointer');
		
		div.getElements('button')[0].addEvent('click',function(cal){
		...
		div.getElements('button')[1].addEvent('click',function(cal){
	...
	today: function(cal) {
		...
		cal.els.each(function(el) {	// then we can set the value to the field
			...
			var today = d.getFullYear()+"-"+month+"-"+todate;
			 el.value = today;
			 el.blur();
		}, this);
