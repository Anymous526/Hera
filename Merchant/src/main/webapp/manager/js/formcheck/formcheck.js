 ...
 * 修改日志
 * 2011-4-9:
 * 1、在form表单的form元素上存储了 FormCheck的对象引用,以后可通过form对象的retrieve("formCheck")来获得FormCheck的对象引用
 * 2、增加了clearAllError()方法，通过该方法清除表单上的所有错误提示
 * 
 */
...
var FormCheck = new Class({
	...
	options : {
		submitReload:0,                      //当提交时是否重新加载需要验证的表单字段（此情况适用于需要验证字段的动态变化） ,默认是不从新加载
			...
			if(el.isChild) {
				...
				*/
				if(rule.match(/%[A-Z0-9\._-{,}]+$/i) || (el.isOk && rule.match(/~[A-Z0-9\._-]+$/i))) {
					...
					if(params.length>0) {
						if(eval(ruleMethod.slice(1)+'(el,'+params+')') == false) {
							el.isOk = false;
						}
					}
					else {
						if(eval(ruleMethod.slice(1)+'(el)') == false) {
							el.isOk = false;
						}
					}
				 ...
				 */
				/**
				if(rule.match(/%.+/i) || (el.isOk && rule.match(/~[A-Z0-9\._-]+$/i))) {
					if(rule.match(/%[A-Z0-9\._-]+$/i) || (el.isOk && rule.match(/~[A-Z0-9\._-]+$/i))) {
						...
						if(eval(ruleMethod.slice(1)+'(el,ruleArgs)') == false) {
							el.isOk = false;
						}
					}
	...
	*/
	validateRegex : function(el, ruleMethod, ruleArgs) {
		...
		if(ruleMethod == 'required') {
			if (this.options.regexp[ruleMethod].test(el.value.trim()) == false)  {
				el.errors.push(msg);
				return false;
			}
		}
	...
	*/
	...
	clearAllError:function() {
		this.validations.each(function(item) {
	...
	*/
	onSubmit: function(event) {
		...
		if(this.options.submitReload==1) {
			this.validations=[];
			this.form.getElements("*[class*=validate]").each(function(el) {
				this.register(el);
			}, this);
