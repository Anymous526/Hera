Ext.onReady(function() {
			...
			Ext.MessageBox.show({
					...
					fn : function(btn) {
						if (btn == 'ok') {
							grid.store.reload();
							id = '';
							Ext.getCmp('smsAgentUnreg').disable();
						}
					}
			});
		} else {
	...
	function smsAgentUnreg() {
		Ext.MessageBox.confirm('短信注销', '确定要注销该条营销短信吗', function(btn) {
			if( btn == 'yes' ){
				Ext.Ajax.request({
					url: contextPath + '/sa.do?method=delete',
					method: 'post',
					params: { id: smsId },
...
//						win.close();
	...
	var addListener = function() {
		...
		selectModel.on('rowselect', function(self, rowIndex, record) {
			...
			if( smsStatusValue == '0' || smsStatusValue == '1' || smsStatusValue == '2'){
				Ext.getCmp('smsAgentUnreg').enable();
			}
		...
		Ext.getCmp('smsAgentUnreg').on('click', function() {
			method = 'delete';
			smsAgentUnreg();
		});
