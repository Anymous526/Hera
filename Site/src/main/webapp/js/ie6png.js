var EvPNG = EvPNG?EvPNG:{
	...
	createVmlStyleSheet : function() {
		...
		if (typeof a.sheet === 'undefined') {
			...
			b = a.sheet;
			var index = b.cssRules.length;
			b.insertRule(this.ns + '\\: *{behavior:url(#default#VML)}', index);
			b.insertRule(this.ns + '\\: shape{position:absolute;}', ++index);
			b.insertRule('img.' + this.ns + '_sizeFinder {behavior:none; border:none; position:absolute; z-index:-1; top:-10000px; visibility:hidden;}', ++index);
		}
		this.styleSheet = b;
	},
	...
	fix : function(a) {
		...
		if ((version >= 5.5) && (version < 7)) {
			...
			for ( var i = 0; i < b.length; i++) {
				if (typeof this.styleSheet.cssRules === 'undefined') {
					this.styleSheet.addRule(b[i], ' {behavior:expression(EvPNG.fixPng(this))}');
				} else {
					this.styleSheet.insertRule(b[i] + ' {behavior:expression(EvPNG.fixPng(this))}', this.styleSheet.cssRules.length);
				}
			}
		}
	},
